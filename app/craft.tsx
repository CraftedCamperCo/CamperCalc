...
interface YouTubeVideo { title: string; videoId: string; published: string; thumbnail: string; }
    ...
    <View style={cal.row}>
      ...
      <View style={[cal.track, { backgroundColor: theme.blurTint === 'dark' ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)', borderColor: theme.blurTint === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)' }]}>
        {Array.from({ length: total }).map((_, i) => {
          ...
          return (
            <View key={i} style={[cal.segment, { left: `${(i / total) * 100}%`, width: `${(1 / total) * 100}%` }, isTaken ? { backgroundColor: theme.blurTint === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.1)' } : { backgroundColor: `${theme.accent}18` }]}>
...
function YouTubeCard({ theme }: { theme: any }) {
  ...
  useEffect(() => {
    fetch(`https://www.youtube.com/feeds/videos.xml?channel_id=UCFt9-XGxQHz4QlIuonHU3uQ`)
      .then(r => r.text())
      .then(xml => {
        ...
        for (const entry of entryMatches) {
          ...
          setVideo({ title, videoId: idMatch[1], published: pubMatch[1].slice(0, 10), thumbnail: `https://img.youtube.com/vi/${idMatch[1]}/hqdefault.jpg` });
          return;
        }
      }).catch(() => {});
  }, []);
  return (
    <TouchableOpacity onPress={() => Linking.openURL(video ? `https://www.youtube.com/watch?v=${video.videoId}` : 'https://www.youtube.com/@craftedcamperco')} activeOpacity={0.8}>
      <GlassCard style={yt.card} noPadding>
        ...
        <View style={yt.body}>
          <View style={yt.header}><View style={yt.youtubeTag}><Text style={yt.youtubeTagText}>▶ YouTube</Text></View><Text style={[yt.channelName, { color: theme.textSecondary }]}>@craftedcamperco</Text></View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.content, { paddingTop: insets.top + 20 }]} showsVerticalScrollIndicator={false}>
        ...
        <AnimatedCard delay={100}>
          <GlassCard style={styles.consultCard} float>
            ...
            <View style={styles.consultItems}>
              {['Full electrical schematic for your layout', 'Every cable size and fuse rating calculated', 'Component list with specific product recommendations', 'Layout optimisation for your van model'].map((item, i) => (
        ...
        <AnimatedCard delay={200}>
          <GlassCard style={styles.card}>
            <Text style={[styles.sectionLabel, { color: theme.accent }]}>FULL BESPOKE BUILDS</Text>
            <Text style={[styles.sectionTitle, { color: theme.text }]}>Let us build your dream camper.</Text>
            <Text style={[styles.sectionHelper, { color: theme.textSecondary }]}>
              From bare metal to finished van — Crafted Camper Co. handles every stage of a professional conversion. Our builds include CNC-cut bespoke furniture, full electrical systems, plumbing, gas, insulation, and interior fit-out. Every van is a one-off, designed around how you actually live and travel.
            </Text>
            <Text style={[styles.sectionHelper, { color: theme.textSecondary }]}>
              Interested in having Crafted build your van? Check our availability below and get in touch for a quote.
            </Text>
            ...
            <TouchableOpacity style={[styles.enquireBtn, { borderColor: `${theme.accent}44` }]} onPress={openWebsite} activeOpacity={0.8}>
              <Text style={[styles.enquireBtnText, { color: theme.accent }]}>Visit craftedcamper.co →</Text>
            </TouchableOpacity>
          </GlassCard>
        </AnimatedCard>
...
const styles = StyleSheet.create({
  ...
  sectionHelper: { fontSize: 13, lineHeight: 19, marginBottom: 12 },
