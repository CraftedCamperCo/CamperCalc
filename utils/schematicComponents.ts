...
export function drawLynxDistributor(
  ...
  const x = cx - w / 2;
  const y = cy - h / 2;
  const f = (n: number) => Math.round(n * (w < 90 ? 0.85 : 1));

  // Fuse holder dimensions — 4 evenly spaced across 80% of body width
  const fuseAreaX = x + w * 0.08;
  const fuseAreaW = w * 0.88;
  const fuseW = fuseAreaW / 4 - 5;
  const fuseH = h * 0.58;
  const fuseTopY = y + h * 0.12;
  const fuseSpacing = fuseAreaW / 4;

  const fuseRatings = ['60A', '60A', '60A', '175A'];
  const fuses: Array<{ fx: number; portX: number; portY: number }> = [];
  for (let i = 0; i < 4; i++) {
    const fx = fuseAreaX + i * fuseSpacing;
    fuses.push({
      fx,
      portX: fx + fuseW / 2,
      portY: fuseTopY + fuseH,
    });
  }

  const fuseSVG = fuses.map(({ fx, portX }, i) => {
    const midX = fx + fuseW / 2;
    return `
  <!-- Fuse holder ${i + 1} — MIDI style, orange body with window -->
  ${rr(fx, fuseTopY, fuseW, fuseH, 3, '#2A2A2A', '#111', 0.8)}
  ${rr(fx + 1.5, fuseTopY + 1.5, fuseW - 3, fuseH - 3, 2, '#D4840A', '#A05500', 0.9)}
  <!-- Fuse window (visible element inside) -->
  ${rr(midX - fuseW * 0.28, fuseTopY + fuseH * 0.25, fuseW * 0.56, fuseH * 0.35, 1.5, '#FFD060', '#CC9000', 0.7)}
  <!-- Fuse rating — rotated for compact holder -->
  <text transform="rotate(-90,${midX},${fuseTopY + fuseH * 0.6})"
    x="${midX}" y="${fuseTopY + fuseH * 0.6}"
    text-anchor="middle" dominant-baseline="central"
    font-family="Arial,sans-serif" font-size="${f(6)}" fill="#FFFFFF" font-weight="bold">${fuseRatings[i]}</text>
  <!-- Top M10 bolt (bus connection) -->
  ${hexNut(midX, fuseTopY + 4, 5, '#888', '#555')}
  <!-- Bottom M10 bolt (output connection) -->
  ${hexNut(midX, fuseTopY + fuseH - 4, 5, '#888', '#555')}`;
  }).join('');

  // Main busbar input bolt positions (left side — connecting to Lynx Power In)
  const busInX = x + 5;

  const svg = `<g id="lynx-${cx}-${cy}">
  <!-- Drop shadow -->
  <rect x="${x + 3}" y="${y + 3}" width="${w}" height="${h}" rx="3" fill="#000" opacity="0.2"/>

  <!-- Main housing body — Victron dark blue -->
  ${rr(x, y, w, h, 3, '#1C3F6E', '#0D2647', 1.5)}

  <!-- Inner recessed panel -->
  ${rr(x + 4, y + 4, w - 8, h - 8, 2, '#142D50', undefined, undefined, 0.6)}

  ${fuseSVG}

  <!-- Busbar-in area (left side — 2 M10 connection bolts) -->
  ${rr(x - 6, cy - 12, 10, 24, 2, '#0D2647', '#091830', 1)}
  ${hexNut(busInX - 3, cy - 6, 5.5, '#C8A840', '#8B6914')}
  ${hexNut(busInX - 3, cy + 6, 5.5, '#C8A840', '#8B6914')}
  ${txt(busInX - 2, cy - 17, 'IN', f(4.5), '#7FB8E0', 'bold')}

  <!-- Label band at bottom -->
  ${rr(x, y + h - 14, w, 14, 2, '#E8750A')}
  ${txt(cx, y + h - 7, 'LYNX DISTRIBUTOR', f(5.5), '#FFFFFF', 'bold')}
</g>`;

  const ports: ComponentPorts = {
    busbar_in:  { x: busInX - 3,  y: cy },
    busbar_neg: { x: cx,           y: y + h },
  };
  fuses.forEach(({ portX, portY }, i) => {
    ports[`fuse_out_${i + 1}`] = { x: portX, y: portY };
  });
