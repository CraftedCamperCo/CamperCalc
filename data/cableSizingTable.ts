import type { CableRunLength } from '@/utils/wiringTypes';

interface CableSizingEntry { gauge: number; maxAmp5m: number; maxAmp10m: number; maxAmp15m: number; maxAmp20m: number; }
...
function totalCableLengthForRun(runLength: CableRunLength): number {
  switch (runLength) { case 'short': return 4; case 'medium': return 8; case 'long': return 14; }
}
...
function maxAmpForGauge(gauge: number, totalLength: number): number {
  ...
  if (totalLength <= 10) { const r = (totalLength - 5) / 5; return entry.maxAmp5m - r * (entry.maxAmp5m - entry.maxAmp10m); }
  if (totalLength <= 15) { const r = (totalLength - 10) / 5; return entry.maxAmp10m - r * (entry.maxAmp10m - entry.maxAmp15m); }
  if (totalLength <= 20) { const r = (totalLength - 15) / 5; return entry.maxAmp15m - r * (entry.maxAmp15m - entry.maxAmp20m); }
...
export function selectCableGauge(currentAmps: number, runLength: CableRunLength): number {
  ...
  for (const entry of CABLE_SIZING_TABLE) { if (maxAmpForGauge(entry.gauge, totalLength) >= currentAmps) return entry.gauge; }
  return 120;
}

export function calculateVoltageDrop(currentAmps: number, cableGauge: number, lengthMetres: number): { voltageDrop: number; percentage: number } {
  const resistivityPerMetre = 0.017 / cableGauge;
  const totalResistance = resistivityPerMetre * lengthMetres * 2;
  const voltageDrop = currentAmps * totalResistance;
  return { voltageDrop: Math.round(voltageDrop * 1000) / 1000, percentage: Math.round((voltageDrop / 12.8) * 1000) / 10 };
}

export function estimateCableLength(runLength: CableRunLength): number {
  switch (runLength) { case 'short': return 1.5; case 'medium': return 3; case 'long': return 6; }
}
